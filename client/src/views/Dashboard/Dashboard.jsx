...
export function EmployeeDashboard() {
  ...
  const [timer, setTimer] = useState(() => {
    return localStorage.getItem("employeeDashboardTimer") || 28800;
  }); // 8 hours in seconds
      ...
      if (storedTimer) {
        ...
        try {
          ...
          localStorage.setItem(
            "employeeDashboardTimer",
            res.data.data[0].remainingTime.toString()
          );
        } catch (error) {
  ...
  useEffect(() => {
    ...
    setIsTimerRunning(currentPath === "/emp-dashboard");
  }, [location]);
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        //   .flat();
        setNotifictions(announcementsData);
      }
    } catch (error) {
