...
export default function AdminCalendar() {
  ...
  const handleDailyClick = async (date) => {
    ...
    // Example: fetch daily report data for the selected date
    try {
      const response = await axios.get(`/api/v1/admin/dailyReport?date=${date}`);
      setDailyReport(response.data.data); // Assuming response.data.data is an array of reports
    } catch (error) {
  ...
  const getEmployeeHistory = async () => {
    try {
      ...
      const historyData = response.data.data.at(-1);

      if (historyData) {
        setStartDate(dayjs(historyData.startDate));
        setEndDate(dayjs(historyData.endDate));
      }
    } catch (error) {
    ...
    <div className='align-center bg-gray-500 w-[50%] scale-[1] '>
      ...
      <dialog open={openOnClick} className='rounded-lg absolute z-50 w-11/12 md:w-3/4 lg:w-1/2 min-h-[300px] top-0 mt-44 bg-black text-white'>
        <section className='flex flex-col items-center justify-center p-4'>
          <h3 className='mt-8 text-center text-lg'>Today's Report</h3>
          ...
          {dailyReport.length !== 0 ? (
            dailyReport.map((report, i) => (
              ...
              >
                <div className='text-center mb-4'>
                  <h1 className='text-2xl font-semibold text-gray-800 mb-1'>{report.projectName}</h1>
                  <a
                    href={report.workUrl}
                    target='_blank'
                  ...
                  <p className='text-sm text-gray-500 mt-2'>{convertToSimpleDate(report.time)}</p>
                </div>
            ...
            <div className='bg-black h-auto w-full rounded-lg overflow-hidden pt-8'>
              ...
              <button className='absolute top-2 right-4' onClick={() => setOpenOnClick(false)}>
                x
              </button>
            </div>
          )}
